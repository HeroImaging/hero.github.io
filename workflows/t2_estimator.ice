<?xml version="1.0" encoding="utf-8"?>
<MICE Version="2021.1.0.0" Timestamp="2021-06-04 13:17:44.4760040">
  <Settings>
    <ID>0f6d064e-8aa0-4ce5-8bcf-036669d7128d</ID>
    <Name>New Process</Name>
    <Description />
    <Version>1.0</Version>
    <Compress>False</Compress>
  </Settings>
  <Nodes>
    <Node ID="D3F5CCd2FFdBDf7D" T="NodeMRIEmulator" V="0.1" X="38" Y="110" FI="">
      <Inputs />
      <Outputs>
        <IO ID="da4bCBf8D52AA0e7" T="Image4DFloat" N="MR Image" MI="1" MA="1" />
        <IO ID="f73CFeb63aC70bcE" T="String" N="Imaging time" MI="1" MA="1" />
      </Outputs>
      <Settings>
        <SN N="Subject" V="Subject04" />
        <SN N="Parameters" V="" />
        <SN N="MatrixX" V="256" />
        <SN N="MatrixY" V="256" />
        <SN N="MatrixZ" V="64" />
        <SN N="DX" V="1" />
        <SN N="DY" V="1" />
        <SN N="DZ" V="2.5" />
        <SN N="PosX" V="0" />
        <SN N="PosY" V="0" />
        <SN N="PosZ" V="0" />
        <SN N="RotX" V="0" />
        <SN N="RotY" V="0" />
        <SN N="RotZ" V="1" />
        <SN N="RotAngle" V="0" />
        <SN N="B0" V="B1_5T" />
        <SN N="Coil" V="HeadCoil" />
        <SN N="Gmax" V="40" />
        <SN N="Sequence" V="SE" />
        <SN N="SeqType" V="dim2D" />
        <SN N="PEDir" V="dirY" />
        <SN N="BWp" V="220" />
        <SN N="NSA" V="1" />
        <SN N="R" V="1" />
        <SN N="PF" V="False" />
        <SN N="TR" V="5000" />
        <SN N="TE" V="20" />
        <SN N="GapZ" V="0" />
        <SN N="TI" V="250" />
        <SN N="RealRecon" V="False" />
        <SN N="FA" V="20" />
        <SN N="RunSingle" V="True" />
      </Settings>
    </Node>
    <Node ID="E0Ac60f5bB2707d1" T="NodeMRIEmulator" V="0.1" X="41" Y="207" FI="">
      <Inputs />
      <Outputs>
        <IO ID="035C13d1500AE44f" T="Image4DFloat" N="MR Image" MI="1" MA="1" />
        <IO ID="5EeE0a0CD81Bb414" T="String" N="Imaging time" MI="1" MA="1" />
      </Outputs>
      <Settings>
        <SN N="Subject" V="Subject04" />
        <SN N="Parameters" V="" />
        <SN N="MatrixX" V="256" />
        <SN N="MatrixY" V="256" />
        <SN N="MatrixZ" V="64" />
        <SN N="DX" V="1" />
        <SN N="DY" V="1" />
        <SN N="DZ" V="2.5" />
        <SN N="PosX" V="0" />
        <SN N="PosY" V="0" />
        <SN N="PosZ" V="0" />
        <SN N="RotX" V="0" />
        <SN N="RotY" V="0" />
        <SN N="RotZ" V="1" />
        <SN N="RotAngle" V="0" />
        <SN N="B0" V="B1_5T" />
        <SN N="Coil" V="HeadCoil" />
        <SN N="Gmax" V="40" />
        <SN N="Sequence" V="SE" />
        <SN N="SeqType" V="dim2D" />
        <SN N="PEDir" V="dirY" />
        <SN N="BWp" V="220" />
        <SN N="NSA" V="1" />
        <SN N="R" V="1" />
        <SN N="PF" V="False" />
        <SN N="TR" V="5000" />
        <SN N="TE" V="80" />
        <SN N="GapZ" V="0" />
        <SN N="TI" V="250" />
        <SN N="RealRecon" V="False" />
        <SN N="FA" V="20" />
        <SN N="RunSingle" V="True" />
      </Settings>
    </Node>
    <Node ID="AB83FBEe2fa5Db3C" T="NodeT2Map" V="0.1" X="660" Y="180" FI="">
      <Inputs>
        <IO ID="DF0f4Ba15184dAca" T="Image4DFloat" N="Echo Series" MI="1" MA="2147483647" />
        <IO ID="c61C2c2aa2F2D333" T="Image4DBool" N="Mask" MI="0" MA="1" />
      </Inputs>
      <Outputs>
        <IO ID="fCb6Ebf20E126a43" T="Image4DFloat" N="T2" MI="1" MA="1" />
      </Outputs>
      <Settings>
        <SN N="Method" V="Linear" />
        <SN N="RunSingle" V="False" />
      </Settings>
    </Node>
    <Node ID="a7d7377ED3DdD4dd" T="NodeImageToMask" V="0.1" X="362" Y="85" FI="">
      <Inputs>
        <IO ID="7fE3efFb184eEac8" T="Image4DFloat" N="Image" MI="1" MA="1" />
      </Inputs>
      <Outputs>
        <IO ID="740C5EB483a84d6f" T="Image4DBool" N="Mask" MI="1" MA="1" />
      </Outputs>
      <Settings>
        <SN N="DivVal" V="0.2" />
        <SN N="DivType" V="Higher" />
        <SN N="RunSingle" V="False" />
      </Settings>
    </Node>
  </Nodes>
  <Connections>
    <CN ID1="DF0f4Ba15184dAca" ID2="da4bCBf8D52AA0e7" />
    <CN ID1="DF0f4Ba15184dAca" ID2="035C13d1500AE44f" />
    <CN ID1="c61C2c2aa2F2D333" ID2="740C5EB483a84d6f" />
    <CN ID1="7fE3efFb184eEac8" ID2="da4bCBf8D52AA0e7" />
  </Connections>
  <Notes>
    <Note ID="ee3D02237FfA5FBd" X="10" Y="10" W="810" H="346" Title="T2 estimation" Text="- Generate two spin echo images with different TE (20, 80 ms).  &#xD;&#xA;- Remove background using a threshold.&#xD;&#xA;- Estimate a T2 map using a linearized singal equation." R="79" G="129" B="189" />
  </Notes>
</MICE>